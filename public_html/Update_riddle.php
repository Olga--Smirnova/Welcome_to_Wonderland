...
/**
  ...
  * November 2014
  * 1 -make a list of all riddles
  * 2 -after choosing a riddle which admin aims to update, save id of that riddle as $_GET['update'] and writes sticky form, filled with data from DB
  * realisation: if(isset($_GET['update']) && is_numeric($_GET['update'])=>show sticky form, filled with data from DB }else{ => show the list of all riddles
*/
...
*/
    ...
    $oHTML = new makeHTML();
    $oForm = new makeHTML($aFormpostriddle);   //form to update riddle
...
//there is no picture
       {
            ...
            $formHTML_updateriddle = $oForm->openForm('update_me')
                                .$oForm->makeInputRowVal('post_title', 'Title:', 'text')
                                .$oForm->makeTextareaRowVal('post', 'Post:')
                                .$oForm->makeInputRowVal('dead_line', 'Deadline:', 'text')
                                .$oForm->makeTextareaRowVal('riddle_answer', 'Answer:')
                                .$oForm->makeRadioRowVal('hide', 'Hide:', '1', 'adm_update_rid', 'hide', 'hidden')
                                .$oForm->makeRadioVal('hide', '2', 'visible')
                                .'</div>'. "\n"
                                .$oForm->submitButton('update_rid', 'Update')
                                .$oForm->closeForm();
// if "Update_btn" was pressed
            if(isset($_POST['update_rid']))
            {
                ...
                {  
                    ...
                    $db->bind('dead_line', $oForm->aFiltered['dead_line']);
                    ...
                    {   
                        ...
                        $error = '*Sorry, riddle was not updated, because you made no changes in it';
                    }
                }else{
...
//there is a picture
            ...
            $row = $db->row('SELECT * FROM tbl_posts WHERE id="'. $_GET['update']. '"');
            $oForm->filterEverything($row); // filter external data
            $formHTML_updateriddle = $oForm->openForm()
...
// $_GET['update'] is not set
        ...
        $aWRPosts = $db->query('SELECT id FROM tbl_posts WHERE username_id=1'); //target all riddles = posts of admin(White Rabbit)
        foreach($aWRPosts as $ind=>$arr)
        {
            foreach($arr as $id=>$num) //$num - id of riddle in tbl_posts
            {
...
$message = (isset($error))? $error: '*Choose riddle to update';
...
?>
        <div id="container">
                ...
                <main class="cf">
                    ...
                    <?php //write the list of links to all riddles
                        if(isset($_GET['update']) && is_numeric($_GET['update']))
                        {
                            echo $formHTML_updateriddle;
                        }else{
                            echo "\n". '<ul class="update">'. "\n". "\t";
