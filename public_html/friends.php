...
/**
  ...
  * November 2014
  * display the feed of all posts of all users (but not admin's posts)
*/
...
############    PREPARATION   #################################################
    ...
    $id = 5; //id of page from tbl_pages
    $db->bind("id", $id);
...
//select all users' posts ()but not admin's)
    $aPosts = $db->query('SELECT id FROM tbl_posts WHERE hide="2" AND username_id!="1" ORDER BY date_entered DESC');
    $_SESSION['users_posts'] = array(); //create array of $_SESSION['users_posts'] (need that later to save post's info)
    ...
    {   
        ...
        {                    
            ...
            array_push($_SESSION['users_posts'], $oHTML->posts($num, $username, $statement, $timestamp, $avatar, $picture, $post_title, $post, $comment_num, $username_id, $answer, $deadline));
        }
    }
...
?>
        <nav>
            <ul class="cf">
                <?php echo $oHTML->makeNavigation($aNavigation, "Friends"); ?>
            </ul>
        </nav>
...
?>
        <div id="container">
                ...
                <?php //display all posts of all users
                    foreach($_SESSION['users_posts'] as $ind=>$item)
                    {
                        echo $item;
                    }
                ?>
